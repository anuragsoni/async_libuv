(** Alternative to
    ...
    Async_unix.Reader}, based on the low latency transport in async_rpc. This uses libuv
    so it works on windows as well. *)
...
val create : ?buf_len:int -> [ `TCP ] Luv.Stream.t -> t
